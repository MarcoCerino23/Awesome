...
import { exercisesData } from './Exercises';
import { getAuth } from "firebase/auth";
import { Picker } from '@react-native-picker/picker'; // Importazione aggiornata del Picker
...
function CreateWorkout({ navigation }) {
    const [workoutName, setWorkoutName] = useState('');
    const [selectedExercises, setSelectedExercises] = useState([]);
    const [users, setUsers] = useState([]); // Array di utenti
    const [selectedUserId, setSelectedUserId] = useState(null); // ID dell'utente selezionato
